...
__global__ void gemm_coalesced(const float *A, const float *B, float *C, float beta, 
    float alpha, unsigned int M, unsigned int N, unsigned int K) {
        ...
        // in one_naive, threadIdx.x moves fastest and was leading to non-consecutive loading from memory
        unsigned int row = blockIdx.x * BLOCKSIZE + (threadIdx.x / BLOCKSIZE);
        unsigned int col = blockIdx.y * BLOCKSIZE + (threadIdx.x % BLOCKSIZE);
    ...
    int main() {
        ...
        dim3 gridDim(CEIL_DIV(M, BLOCKSIZE), CEIL_DIV(N, BLOCKSIZE), 1); // gird sizing is done based on tile size, not block size
        dim3 blockDim(BLOCKSIZE * BLOCKSIZE);
        gemm_coalesced<<<gridDim, blockDim>>>(dA, dB, dC, beta, alpha, M, N, K);
