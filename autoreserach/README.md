...
```text
autoreserach/candidate/candidate_gemm.cu
```
...
```bash
cd <GPU-repository>/autoreserach
./trusted/run_competition.sh
```

Results are written under `autoreserach/results/<UTC timestamp>/`.
...
The workflow template in `github/gemm-autoresearch.yml` checks out verifier
code from the PR base commit rather than the candidate commit. It must be copied
to the repository root as `.github/workflows/gemm-autoresearch.yml`.
